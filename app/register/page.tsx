...
function getCroppedImg(
  ...
  canvas.width = Math.max(0, Math.floor(crop.width * pixelRatio));
  canvas.height = Math.max(0, Math.floor(crop.height * pixelRatio));
...
const Register: React.FC = () => {
  ...
  const handleCropConfirm = async () => {
    ...
    try {
      ...
      if (error) throw new Error("เกิดข้อผิดพลาดในการอัปโหลดรูป");
